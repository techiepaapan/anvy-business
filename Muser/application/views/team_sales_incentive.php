...
<li><a href="<?php echo base_url();?>welcome/upgradeuser"><i class="fa fa-sitemap" aria-hidden="true"></i> Upgrade</a></li>
   ...
   <script>
	...
	function getdata(base_url,from,to){
		...
		$("#walappend").html("");
		//alert(base_url+"welcome/getLSI?from="+from+"&to="+to);
		$.ajax({
			 url: base_url+"welcome/getTSI", // point to server-side controller method
		     ...
		     success: function (jsn) {
		    		...
		    		for(var i=0;i<jsn.length;++i){
		    			...
		    			var payin_date=jsn[i].payin_date;
		    			var lbv=jsn[i].lbv;
		    			var rbv=jsn[i].rbv;
		    			var match=jsn[i].bmatch;
		    			var pay_amount=jsn[i].pay_amount;
			    		...
			    		$("#walappend").append('<tr>'+
			    				...
			    				'<td>'+rbv+'</td>'+
			    				'<td>'+match+'</td>'+
			    				'<td><i class="fa fa-inr" aria-hidden="true"></i> '+pay_amount+'</td>'+
			    				'</tr>'
			    		);
		    		}	
		     }
