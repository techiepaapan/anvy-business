...
(select sum(bmatch) from user_payin upn where upn.u_id=ul.u_id) as pbv from user_extradetails ue, user_login ul
												where ue.referral_id ='$id' and ul.u_id=ue.u_id");
					foreach ($query->result() as $row)
					{$i++;
					 
					 $date=trim( implode('-', array_reverse(explode('-', $row->u_joindate))));
						?>
						<tr>
							<td><?php echo $i;?></td> 
							...
							<td><?php echo $date;?></td>
							<td><img src="<?php echo base_url().'/'.getrank($row->pbv);?>" style="width: 45px;"/></td>
						</tr>
						<?php
					}
					
					function getrank($rankbv){
					    if($rankbv=="" || $rankbv==null || $rankbv<0){
					        $rankbv=0;
					    }
					    $image="images/tree-person.png";
