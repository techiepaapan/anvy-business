...
class DbOperation
{
    ...
    private function isEmailexist($email){
        $stmt = $this->con->prepare("SELECT id FROM token WHERE email = ?");
    ...
    public function getAllTokens(){
        $stmt = $this->con->prepare("SELECT token FROM token order by id desc limit 1000");
        $stmt->execute(); 
    ...
    public function getTokenByEmail($email){
        $stmt = $this->con->prepare("SELECT token FROM token WHERE email = ?");
        $stmt->bind_param("s",$email);
        $stmt->execute(); 
    ...
    public function getAllDevices(){
        $stmt = $this->con->prepare("SELECT * FROM token order by id desc limit 1000");
        $stmt->execute();
        $result = $stmt->get_result();
        return $result; 
    }
