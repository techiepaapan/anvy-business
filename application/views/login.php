...
<body>
	<div id="inner">
		...
		<!--login_logo starts-->
		<div class="login_logo" style="padding-top: 30px;"><a href="<?php echo base_url(); ?>">
				<h1>ANVY BUSINESS</h1>
			</a></div>
		<!--login logo ends-->
		...
		<div class="login_box">
			<div class="login-top">
				<p class="error-msg" id="referror" style="color: red;height: 20px;"></p>
			</div>
			<div class="login-cnt">
				<form id="loginform">
					<fieldset>
						...
						<p>
							<a href="<?php echo base_url(); ?>welcome/forgot"> <span class="spn">Forgot Password?</span></a>
						</p>
		...
		<div class="clear"></div>
		<!--link to home page-->
		<div class="login_homelink"><a href="<?php echo base_url(); ?>">Back to Anvy Business Services</a></div>
	</div>
			...
			$("#submit").click(function() {
				...
				if (user == "" || pwd == "") {
					...
					$.getJSON(baseurl + "welcome/getlogin?user=" + user + "&pwd=" + pwd, function(jsn) {
						$("#referror").text("");
						if (jsn[0].res == 1) {
							if (jsn[0].type == 'W') { //alert(jsn[0].pin+" - "+jsn[0].uid);
								//window.location.href=baseurl;
								var ur = baseurl + 'Muser/welcome/userloggedin?uid=' + jsn[0].uid + '&pin=' + jsn[0].pin + '&name=' + jsn[0].userx;
							} else {
								var ur = baseurl;
							...
							$("#referror").text("Login Blocked!!! Please contact admin to activate");
							$("#user").val("");
							$("#password").val("");
						} else {
